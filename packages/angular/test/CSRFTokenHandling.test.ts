...
import { AngularXhrClient, DEFAULT_CSRF_TOKEN_KEY } from "../src/index.js";

const TOKEN = "abc-123";
const TOKEN_KEY = DEFAULT_CSRF_TOKEN_KEY;
const FETCH_URL = "https://test.testing.com";

describe("AngularXhrClient Automatic CSRF Handling Tests", () => {
  ...
  function buildClient(options: ODataHttpClientOptions = { useCsrfProtection: true, csrfTokenFetchUrl: FETCH_URL }) {
    ...
    return new AngularXhrClient(httpClientMock, options);
  }
  ...
  test("a request is only ever retried once, so a server that keeps demanding a new token still fails", async () => {
    ...
    client = new AngularXhrClient(httpClientMock, { useCsrfProtection: true, csrfTokenFetchUrl: FETCH_URL });
  ...
  test("missing csrfTokenFetchUrl throws immediately when constructing the client", () => {
    ...
    expect(() => new AngularXhrClient(httpClientMock, { useCsrfProtection: true })).toThrow(
      /the URL must be supplied via attribute \[csrfTokenFetchUrl\]/,
    );
    expect(() => new AngularXhrClient(httpClientMock, { useCsrfProtection: true, csrfTokenFetchUrl: "  " })).toThrow(
      /the URL must be supplied via attribute \[csrfTokenFetchUrl\]/,
    );
  });
