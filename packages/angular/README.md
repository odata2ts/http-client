...
`AngularXhrClient` is an injectable service (`providedIn: "root"`) that wraps the `HttpClient` instance
provided through Angular's dependency injection - it does not create its own `HttpClient` or configure the
`HttpClientModule`/`provideHttpClient` setup, which remains the consuming application's responsibility.
...
- automatic CSRF token handling for the OData V2/V4 handshake (`useCsrfProtection` / `csrfTokenFetchUrl`,
...
- **streaming** in either direction (`getStream`, `createStream` / `updateStream`): this client is meant
  for `HttpClient`'s default `XhrBackend`, which has no streaming API. Those calls are refused with an
  error - use the [Fetch Client](https://www.npmjs.com/package/@odata2ts/http-client-fetch) for streams.

### CSRF token handling

Because `AngularXhrClient` is constructed by Angular's own DI container (`providedIn: "root"`), its options
cannot be passed as a plain constructor argument the way the other odata2ts HTTP clients accept them - there
is no `new AngularXhrClient(options)` call for Angular to intercept. Provide the `ANGULAR_XHR_CLIENT_OPTIONS`
injection token instead:

```ts
import { ANGULAR_XHR_CLIENT_OPTIONS } from "@odata2ts/http-client-angular";

providers: [
  {
    provide: ANGULAR_XHR_CLIENT_OPTIONS,
    useValue: { useCsrfProtection: true, csrfTokenFetchUrl: "/odata/my-service/" },
  },
];
```

With this in place, `AngularXhrClient` fetches a token via a `GET` to `csrfTokenFetchUrl` (header
