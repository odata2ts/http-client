  ...
  private convertError(error: Error): AxiosODataClientError {
    if ((error as AxiosError).isAxiosError) {
      ...
      if (axiosError.response) {
        const msg = `Server responded with error: ${this.getErrorMessage(axiosError) || ""}`;
        return new AxiosODataClientError(msg, axiosError.response.status, error, axiosError);
      } else {
        const msg = axiosError.request ? "No response from server!" : "Axios Error: no request was sent!";
        return new AxiosODataClientError(msg, undefined, error, axiosError);
      }
    }
    return new AxiosODataClientError("Error", undefined, error);
  }
